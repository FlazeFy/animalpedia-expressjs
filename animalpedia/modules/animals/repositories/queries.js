...
function getAllNews(req, res, path, page, pageSize){
    // Query Builder
    const baseTable = 'news'

    selectTemplate = builders.getTemplateSelect("content_info", baseTable, null)
    propsTemplate = builders.getTemplateSelect("properties_time", null, null)
	activeTemplate = builders.getTemplateLogic("active")
	order = builders.getTemplateOrder("dynamic_data", baseTable, "news_name")

    const offset = (page - 1) * pageSize
    const sqlStatement = "SELECT " + selectTemplate + ", news_tag, news_body, news_time_read, news_img_url, " + propsTemplate + " " +
        "FROM " + baseTable + " " +
        "WHERE " + activeTemplate + " " +
        "ORDER BY " + order + " " +
        "LIMIT "+pageSize+" OFFSET "+offset
